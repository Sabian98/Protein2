...
flags.DEFINE_bool("reconstruct", True, "Reconstruct image")
flags.DEFINE_bool("generate", True, "Generate image from latent")
...
def train(n_samples, batch_size, n_epochs):
	autoencoder, discriminator, generator, encoder, decoder = create_model(input_dim=222, latent_dim=FLAGS.latent_dim)
	...
	for epoch in np.arange(1, n_epochs + 1):
		...
		for batch in np.arange(len(x) / batch_size):
			...
			samples = x[start:end]
			autoencoder_history = autoencoder.fit(x=samples, y=samples, epochs=1, batch_size=batch_size, validation_split=0.1, verbose=0)
			if FLAGS.adversarial:
