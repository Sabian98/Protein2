...
class AAN():
    ...
    def _genEncoderModel(self, img_shape, encoded_dim):
        ...
        encoder.add(Dense(1000, input_shape=(img_shape,), activation='relu', kernel_initializer=initializer,
    ...
    def generateImages(self, test_x, train_x):

        # latents = 5*np.random.normal(size=(len(test_x), self.encoded_dim))
        # lat2=self.encoder_discriminator.predict(test_x)
        latents = self.encoder.predict(test_x)

        # imgs = self.decoder.predict(latents)
        history = self.autoencoder.fit(train_x, train_x,
                                  epochs=1000,
                                  batch_size=100,
                                  shuffle=True,
                                  validation_split=0.1,
                                  verbose=0)
        latents = self.autoencoder.predict(test_x)
        return  latents
...
if __name__ == '__main__':
    ...
    train_after_energy, EnergyAfterTrain = train[:, 0:222], train[:, 222:223]
    # print(train_after_enegy.shape)
    # print(EnergyAfterTrain.shape)

    # # separate Energy from test data
    test_after_energy, EnergyAfterTest = test[:, 0:222], test[:, 222:223]
    # print(test_after_enegy.shape)
    # print(EnergyAfterTest.shape)

    train_x = np.array(train_after_energy)
    test_x = np.array(test_after_energy)

    ann = AAN(encoded_dim=2)
    # print(train_x[0:2])
    ann.train(train_x)
    # # generated = ann.generateImages()
    latents= ann.generateImages(test_x, train_x)
    # print (latents)
